...
class Friend:
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO friends (first_name, last_name, occupation, team_id, created_at, updated_at) VALUES (%(fname)s, %(lname)s, %(occ)s, %(team_id)s NOW(), NOW());"
        # data is a dictionary that will be passed into the save method from server.py
        return connect_to_mysql('first_flask').query_db(query, data)
    ...
    @classmethod
    def update(cls, data):
        query = """UPDATE friends SET first_name=%(first_name)s, last_name=%(last_name)s, occupation=%(occupation)s, created_at=NOW(), 
